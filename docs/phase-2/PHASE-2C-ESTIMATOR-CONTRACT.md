...
The estimator-effective graph is built after outcome-policy filtering. It must contain at least two models and exactly one connected component. The estimator never silently selects a largest component.
...
Stable domain error codes distinguish missing required fields, invalid outcomes or model IDs, zero likelihood rows, insufficient models, models dropped by an outcome policy, disconnected graphs, decisive Bradley-Terry separation, an unidentifiable Davidson tie parameter, optimizer failure, and nonfinite results. A decisive Bradley-Terry fit requires a strongly connected directed win graph; Davidson requires at least one ordinary tie and one decisive outcome. Failed fits do not return partial rankings.
